#!/usr/bin/env python3
"""Simple hyperparameter tuner for the droplet classifier."""
...
def _start_wandb_trial(base_args: argparse.Namespace, classes: List[str], cfg: Dict, trial_id: int):
    ...
    run = wandb.init(
        ...
        group=base_args.wandb_group or "tuning",
        job_type="tune",
        config=base_cfg,
        reinit=False,
    )
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="Hyperparameter tuner for droplet CNN (Python).")
...
def main() -> None:
    ...
    out_dir = Path(args.output_dir) / f"tune_{time.strftime('%Y%m%d_%H%M%S')}"
    ...
    trials: List[Dict] = []
    best = None
    for i, (lr, wd, bs) in enumerate(grids, start=1):
        cfg = {"lr": lr, "weight_decay": wd, "batch_size": bs}
        print(f"[Trial {i}/{len(grids)}] lr={lr} wd={wd} bs={bs}")
        wandb_run = _start_wandb_trial(args, data_splits[3], cfg, i)  # classes at index 3
        result = run_trial(i, cfg, data_splits, device, args, wandb_run=wandb_run)
        if wandb_run:
            import wandb  # type: ignore
    ...
    # Rebuild model with best config and export.
    ...
    print(f"Tuning complete. Best trial {best['trial_id']} | val acc {best['metrics']['best_val_acc']:.3f} | test acc {best['metrics']['test_acc']:.3f}")
